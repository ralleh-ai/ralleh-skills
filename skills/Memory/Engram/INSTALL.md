...
Use this file when a bot needs to install, configure, verify, or operate ENGRAM on a host.

Read [`README.md`](README.md) first for what ENGRAM is and when to use it. Use [`DOCTOR.md`](DOCTOR.md) if install or verification fails.
...
> Read `README.md` and `INSTALL.md`. Install ENGRAM, configure it as a local service if systemd is available, run the smoke tests, and report the exact verification output. If anything fails, switch to `DOCTOR.md` and report the blocker.
